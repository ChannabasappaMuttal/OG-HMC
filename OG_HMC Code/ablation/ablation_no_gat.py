# ============================================================
# ABLATION A1 — HierFashion without GAT
# Replace GAT with a simple MLP on label embeddings
# Tests: "How much does the graph attention network contribute?"
# Everything else identical to final.py
# ============================================================
...
logger = get_logger("ablation_no_gat")


class MLPLabelModule(nn.Module):
    """Replaces GAT — plain 2-layer MLP on learnable node embeddings."""

    def __init__(self, num_nodes, embed_dim=300, out_dim=512):
        super().__init__()
    ...
    def forward(self):
        return self.norm(self.mlp(self.node_embeddings))
...
class AblationNoGATLightning(pl.LightningModule):
    ...
    def __init__(self, hierarchy, adj_matrix, pos_weight, learning_rate=LR):
        ...
        # ❌ NO GAT — replaced with MLP
        self.label_module   = MLPLabelModule(hierarchy.num_nodes, 300, 512)
    ...
    def forward(self, images, targets=None):
        ...
        label_repr      = self.label_module()                      # [N, 512] (no GAT)
        attended_visual = self.label_guided_attn(fmaps, label_repr)
        visual_512      = self.visual_proj(attended_visual)
        cross_feat      = self.cross_attention(attended_visual, label_repr)
...
if __name__ == "__main__":
    ...
    model = AblationNoGATLightning(hierarchy, adj, pos_weight)
    analyze_model(model, device)

    ckpt_cb = ModelCheckpoint(
        monitor="val_loss", mode="min", save_top_k=1,
        dirpath="checkpoints/ablation_no_gat",
        filename="ablation_no_gat-{epoch:02d}-{val_loss:.4f}"
    )
    trainer = pl.Trainer(
        max_epochs=EPOCHS,
        ...
        callbacks=[ckpt_cb,
                   EarlyStopping(monitor="val_loss", mode="min",
                                 patience=PATIENCE)],
        logger=CSVLogger("training_logs", name="Ablation_NoGAT"),
    )
    ...
    if best_ckpt:
        model = AblationNoGATLightning.load_from_checkpoint(best_ckpt, hierarchy=hierarchy, adj_matrix=adj, pos_weight=pos_weight)
        model.to(device)

    os.makedirs("results", exist_ok=True)
    evaluate_hierarchical(model, test_dl, device, model.loss_fn,
                          save_path="results/ablation_no_gat_test_results.csv")
