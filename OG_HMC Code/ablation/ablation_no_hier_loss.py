# ============================================================
# ABLATION A3 — HierFashion without Hierarchical Loss
# Replace CompositeHierarchicalLoss with plain BCE per head
# Tests: "How much do consistency + path coherence losses contribute?"
# Full architecture intact — only loss changes
# ============================================================
...
from core.common import (
    ...
    build_category_attribute_mask, get_logger, load_hierarchy,
...
logger = get_logger("ablation_no_hier_loss")


# ---- Plain multi-head BCE (no consistency, no path coherence) ----
...
class PlainMultiHeadBCE(nn.Module):
    ...
    def forward(self, predictions, targets, return_components=False):
        L_cat   = self.bce_cat(predictions["categories"],    targets["categories"].float())
        L_sub   = self.bce_sub(predictions["subcategories"], targets["subcategories"].float())
        L_group = self.bce_group(predictions["attr_groups"], targets["attr_groups"].float())
        L_attr  = self.bce_attr(predictions["attributes"],   targets["attributes"].float())

        CHL = (L_cat + L_sub + L_group + L_attr) / 4

        if return_components:
            # Return zeros for consistency and path so evaluate_hierarchical still works
            zero = torch.tensor(0.0, device=CHL.device)
            return CHL, CHL, zero, zero
        return CHL


# ---- Reuse architecture blocks from final.py ----
...
class Phase2LabelGraphModule(nn.Module):
    ...
    def forward(self):
        ...
        return self.norm2(self.gat2(x, self.edge_index))
...
class CrossAttentionFusion(nn.Module):
    ...
    def forward(self, visual, graph_nodes):
        ...
        k = self.graph_proj(graph_nodes).unsqueeze(0).expand(B, -1, -1)
        o, _ = self.attn(q, k, k)
        return self.norm(self.out_proj(o.squeeze(1)))
...
class AblationNoHierLossLightning(pl.LightningModule):
    ...
    def __init__(self, hierarchy, adj_matrix, pos_weight, learning_rate=LR):
        ...
        # ❌ Plain BCE — no consistency or path coherence terms
        ...
        mask = build_category_attribute_mask(hierarchy)
        self.register_buffer("cat_attr_mask", mask)
    ...
    def forward(self, images, targets=None):
        ...
        cat_probs = torch.sigmoid(predictions["categories"])
        predictions["attributes"] = predictions["attributes"] * torch.matmul(cat_probs, self.cat_attr_mask)
...
if __name__ == "__main__":
    ...
    model = AblationNoHierLossLightning(hierarchy, adj, pos_weight)
    analyze_model(model, device)

    ckpt_cb = ModelCheckpoint(
        monitor="val_loss", mode="min", save_top_k=1,
        dirpath="checkpoints/ablation_no_hier_loss",
        filename="ablation_no_hier_loss-{epoch:02d}-{val_loss:.4f}"
    )
    trainer = pl.Trainer(
        max_epochs=EPOCHS,
        ...
        callbacks=[ckpt_cb,
                   EarlyStopping(monitor="val_loss", mode="min",
                                 patience=PATIENCE)],
        logger=CSVLogger("training_logs", name="Ablation_NoHierLoss"),
    )
    ...
    if best_ckpt:
        model = AblationNoHierLossLightning.load_from_checkpoint(best_ckpt, hierarchy=hierarchy, adj_matrix=adj, pos_weight=pos_weight)
        model.to(device)

    os.makedirs("results", exist_ok=True)
    evaluate_hierarchical(model, test_dl, device, model.loss_fn,
                          save_path="results/ablation_no_hier_loss_test_results.csv")
