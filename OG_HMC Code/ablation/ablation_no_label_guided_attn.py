# ============================================================
# ABLATION A4 — HierFashion without Label-Guided Spatial Attention
# Replace LabelGuidedSpatialAttention with plain global average pool
# Tests: "How much does label-guided spatial attention contribute?"
# Full GAT + cross-attention + hierarchical loss intact
# ============================================================
...
logger = get_logger("ablation_no_label_guided_attn")
...
class AblationNoLabelGuidedAttnLightning(pl.LightningModule):
    ...
    def __init__(self, hierarchy, adj_matrix, pos_weight, learning_rate=LR):
        ...
        # ❌ NO label-guided spatial attention — use global average pool instead
        self.global_pool     = nn.AdaptiveAvgPool2d((1, 1))
        self.gap_norm        = nn.LayerNorm(2048)
    ...
    def forward(self, images, targets=None):
        ...
        # ❌ Plain GAP instead of label-guided spatial attention
        visual_gap  = self.global_pool(fmaps).flatten(1)    # [B, 2048]
        visual_gap  = self.gap_norm(visual_gap)

        visual_512  = self.visual_proj(visual_gap)
        cross_feat  = self.cross_attention(visual_gap, graph_nodes)
...
if __name__ == "__main__":
    ...
    model = AblationNoLabelGuidedAttnLightning(hierarchy, adj, pos_weight)
    analyze_model(model, device)

    ckpt_cb = ModelCheckpoint(
        monitor="val_loss", mode="min", save_top_k=1,
        dirpath="checkpoints/ablation_no_label_guided_attn",
        filename="ablation_no_label_guided_attn-{epoch:02d}-{val_loss:.4f}"
    )
    trainer = pl.Trainer(
        max_epochs=EPOCHS,
        ...
        callbacks=[ckpt_cb,
                   EarlyStopping(monitor="val_loss", mode="min",
                                 patience=PATIENCE)],
        logger=CSVLogger("training_logs", name="Ablation_NoLabelGuidedAttn"),
    )
    ...
    if best_ckpt:
        model = AblationNoLabelGuidedAttnLightning.load_from_checkpoint(best_ckpt, hierarchy=hierarchy, adj_matrix=adj, pos_weight=pos_weight)
        model.to(device)

    os.makedirs("results", exist_ok=True)
    evaluate_hierarchical(model, test_dl, device, model.loss_fn,
                          save_path="results/ablation_no_label_guided_attn_test_results.csv")
