# ============================================================
# ABLATION A7 — HierFashion without Hierarchical Masking
# Full model intact — cat_attr_mask NOT applied to attributes
# Tests: "How much does category-guided attribute masking contribute?"
...
logger = get_logger("ablation_no_hier_mask")
...
class AblationNoHierMaskLightning(pl.LightningModule):
    ...
    def __init__(self, hierarchy, adj_matrix, pos_weight, learning_rate=LR):
        ...
        # ❌ No cat_attr_mask registered — masking is disabled
    ...
    def forward(self, images, targets=None):
        ...
        # ❌ Attribute predictions NOT masked by category probabilities
...
if __name__ == "__main__":
    ...
    model = AblationNoHierMaskLightning(hierarchy, adj, pos_weight)
    analyze_model(model, device)

    ckpt_cb = ModelCheckpoint(
        monitor="val_loss", mode="min", save_top_k=1,
        dirpath="checkpoints/ablation_no_hier_mask",
        filename="ablation_no_hier_mask-{epoch:02d}-{val_loss:.4f}"
    )
    trainer = pl.Trainer(
        max_epochs=EPOCHS,
        ...
        callbacks=[ckpt_cb,
                   EarlyStopping(monitor="val_loss", mode="min",
                                 patience=PATIENCE)],
        logger=CSVLogger("training_logs", name="Ablation_NoHierMask"),
    )
    ...
    if best_ckpt:
        model = AblationNoHierMaskLightning.load_from_checkpoint(best_ckpt, hierarchy=hierarchy, adj_matrix=adj, pos_weight=pos_weight)
        model.to(device)

    os.makedirs("results", exist_ok=True)
    evaluate_hierarchical(model, test_dl, device, model.loss_fn,
                          save_path="results/ablation_no_hier_mask_test_results.csv")
