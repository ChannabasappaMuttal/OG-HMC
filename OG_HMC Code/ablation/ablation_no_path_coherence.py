# ============================================================
# ABLATION A6 — HierFashion without Path Coherence Loss Term
# Full model intact — only L_path removed from CHL
# CHL = L_bce + lambda_consistency * L_consistency  (no path)
# Tests: "How much does end-to-end path coherence loss contribute?"
# ============================================================
...
logger = get_logger("ablation_no_path")


# ── Loss: CHL without path coherence term ────────────────────
class CHL_NoPath(nn.Module):
    """BCE (with focal for attrs) + consistency — NO path coherence term."""

    def __init__(self, pos_weight_cat=None, pos_weight_sub=None,
                 pos_weight_group=None, lambda_consistency=0.1,
...
class AblationNoPathLightning(pl.LightningModule):
...
if __name__ == "__main__":
    ...
    model = AblationNoPathLightning(hierarchy, adj, pos_weight)
    analyze_model(model, device)

    ckpt_cb = ModelCheckpoint(
        monitor="val_loss", mode="min", save_top_k=1,
        dirpath="checkpoints/ablation_no_path_coherence",
        filename="ablation_no_path_coherence-{epoch:02d}-{val_loss:.4f}"
    )
    trainer = pl.Trainer(
        max_epochs=EPOCHS,
        ...
        callbacks=[ckpt_cb,
                   EarlyStopping(monitor="val_loss", mode="min",
                                 patience=PATIENCE)],
        logger=CSVLogger("training_logs", name="Ablation_NoPath"),
    )
    ...
    if best_ckpt:
        model = AblationNoPathLightning.load_from_checkpoint(best_ckpt, hierarchy=hierarchy, adj_matrix=adj, pos_weight=pos_weight)
        model.to(device)

    os.makedirs("results", exist_ok=True)
    evaluate_hierarchical(model, test_dl, device, model.loss_fn,
                          save_path="results/ablation_no_path_coherence_test_results.csv")
